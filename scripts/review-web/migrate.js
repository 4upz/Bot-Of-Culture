...
const { COLLECTIONS, checksum, decode, planCollection, assertArchive, transition } = require('./migration-core')
...
async function run() {
  ...
  try {
    ...
    if (mode === 'dryrun') {
      ...
      const options = { productionGuildId: value('production-guild'), testGuildIds: value('test-guilds') === 'none' ? [] : value('test-guilds').split(','), legacyPublicConfirmed: args.includes('--legacy-public-confirmed'), legacyOriginConfirmed: args.includes('--legacy-origin-confirmed') }
