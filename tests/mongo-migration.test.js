...
test('approved older winner overrides fail closed and restore every BSON original', { skip: !uri }, async t => {
  ...
  const name = `${databaseName}_overrides`, db = client.db(name)
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'boc-winner-test-'))
  t.after(async () => { await db.dropDatabase(); await client.close(); await fs.rm(dir, { recursive: true, force: true }) })
  const collection = db.collection('MovieReview')
  const originals = [fixture(101, {movieId:'123',comment:'approved older production',isPrivate:false}), fixture(102,{movieId:'123',guildId:'test',_createdAt:new Date('2022-01-01'),isPrivate:true,score:new BSON.Double(5)}),fixture(103,{movieId:'other',userId:'other',isPrivate:false})]
  await collection.insertMany(originals)
  const source = await collection.find({movieId:'123'},{promoteValues:false}).toArray()
  const override = {collection:'MovieReview',userId:source[0].userId,mediaId:'123',canonicalReviewId:String(originals[0]._id),expectedSources:source.map(doc=>({originalReviewId:String(doc._id),sourceChecksum:checksum(doc)}))}
  const overridesPath = path.join(dir,'overrides.json')
  let manifestPath = path.join(dir,'stale.json')
  const cli = (mode, extra=[]) => exec(process.execPath,['scripts/review-web/migrate.js',mode,'--database',name,'--manifest',manifestPath,...extra],{cwd:path.resolve(__dirname,'..'),env:{...process.env,MIGRATION_DATABASE_URL:uri}})
  const auditArgs = ['--production-guild','production','--test-guilds','test','--legacy-public-confirmed','--legacy-origin-confirmed','--winner-overrides',overridesPath]
  await fs.writeFile(overridesPath,JSON.stringify([override]),{mode:0o600})
  await collection.updateOne({_id:originals[0]._id},{$set:{comment:'stale'}})
  await assert.rejects(cli('dryrun',auditArgs))
  const blocked = JSON.parse(await fs.readFile(manifestPath,'utf8'))
  assert.equal(blocked.status,'BLOCKED')
  await assert.rejects(cli('apply',['--writers-paused']))
  await collection.replaceOne({_id:originals[0]._id},originals[0])
  manifestPath = path.join(dir,'unmatched.json')
  await fs.writeFile(overridesPath,JSON.stringify([{...override,collection:'GameReview'}]))
  await assert.rejects(cli('dryrun',auditArgs))
  assert.equal(JSON.parse(await fs.readFile(manifestPath,'utf8')).status,'BLOCKED')
  await fs.writeFile(overridesPath,JSON.stringify([override]))
  manifestPath = path.join(dir,'valid.json')
  await cli('dryrun',auditArgs)
  const manifest=JSON.parse(await fs.readFile(manifestPath,'utf8'))
  const group=manifest.plan.groups.find(group=>group.mediaId==='123')
  assert.equal(group.canonicalReviewId,String(originals[0]._id))
  assert.equal(group.entries.find(entry=>entry.recordKind==='CANONICAL_BEFOREIMAGE').originalReviewId,String(originals[0]._id))
  assert.deepEqual(manifest.plan.options.winnerOverrides,[override])
  assert.equal(manifest.planChecksum,checksum(manifest.plan))
  // A missing original on first apply is a stale audit, not an interrupted migration.
  await collection.deleteOne({_id:originals[1]._id})
  await assert.rejects(cli('apply',['--writers-paused']))
  assert.equal(checksum(await collection.findOne({_id:originals[0]._id},{promoteValues:false})),checksum(originals[0]))
  assert.equal(await db.collection('ReviewMigrationArchive').countDocuments({operationState:'APPLIED'}),0)
  await collection.insertOne(originals[1])
  const added=fixture(104,{movieId:'123'})
  await collection.insertOne(added)
  await assert.rejects(cli('apply',['--writers-paused']))
  assert.equal(await collection.countDocuments(),4)
  assert.equal(checksum(await collection.findOne({_id:originals[0]._id},{promoteValues:false})),checksum(originals[0]))
  await collection.deleteOne({_id:added._id})
  await cli('apply',['--writers-paused'])
  await cli('apply',['--writers-paused'])
  await cli('verify')
  assert.equal(await collection.countDocuments(),2)
  const winner=await collection.findOne({_id:originals[0]._id},{promoteValues:false})
  assert.equal(winner.comment,'approved older production')
  assert.equal(winner.isPrivate,true)
  assert.equal(winner.score._bsontype,'Int32')
  assert.equal(await db.collection('ReviewMigrationArchive').countDocuments(),3)
  await cli('rollback',['--writers-paused'])
  await cli('rollback',['--writers-paused'])
  for(const original of originals) {
    const restored=await collection.findOne({_id:original._id},{promoteValues:false})
    assert.equal(checksum(restored),checksum(original))
    assert.equal(restored.futureField.safeLong._bsontype,'Long')
    assert.equal(restored.futureField.integralDouble._bsontype,'Double')
    assert.equal(restored.futureField.decimal._bsontype,'Decimal128')
  }
